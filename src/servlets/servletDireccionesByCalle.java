 ...
 */
@WebServlet("/servletDireccionesByCalle")
public class servletDireccionesByCalle extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public servletDireccionesByCalle() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String descLoc = request.getParameter("DescLocIn");

		if (calle == null || calle.trim().isEmpty()
		        || altura == null || altura.trim().isEmpty()
		        || descLoc == null || descLoc.trim().isEmpty()) {
		    request.getRequestDispatcher("WEB-INF/errorBusquedaDireccionesByCalle.jsp").forward(request, response);
		    return;
		}
		...
		if (d!=null) {
			...
			request.getRequestDispatcher("WEB-INF/muestreoDireccionesByCalle.jsp").forward(request, response);
		}
		else {
			request.getRequestDispatcher("WEB-INF/errorBusquedaDireccionesByCalle.jsp").forward(request, response);
		}
