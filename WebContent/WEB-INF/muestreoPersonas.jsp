					...
					<table class="table table-hover table-striped table-sm border-secondary border border-5 rounded-3 bg-light">	
					  <thead  class="table-dark">
					    <tr>
					      ...
					      <th colspan="2" ><a href="ABMCPersonas/Agregar/agregarPersonas.html"><button type="button" class="btn btn-light border border-dark"  style="width:210px; height:35px">Agregar</button></a></th>
					    </tr>
					  </thead>  
					  <tbody>
						<% 
						...
						<tr>
							...
							<td><%=per.getRol().getDescripcion()%></td>
							
							<td><%=per.getValuacionPromedio() %></td>
							
							<form action="ABMCPersonas/Modificacion/modificarPersonas.jsp" method="post">
								...
								<input type="hidden" name="IdRolIn" value="<%=per.getRol().getDescripcion() %>" />
								<input type="hidden" name="valuacion" value="<%=per.getValuacionPromedio()%>" />
							<td><button  value="<%=per.getDni() %>" name="DNIin" type="submit" class="btn btn-light border border-dark"  style="width:100px; height:35px">Editar</button></td>
						</form>
						...
						<form action="ABMCPersonas/Eliminar/eliminarPersonas.jsp" method="get" >
								...
								<input type="hidden" name="IdRolIn" value="<%=per.getRol().getDescripcion() %>" />
								<input type="hidden" name="valuacion" value="<%=per.getValuacionPromedio()%>" />
							<td><button  value="<%=per.getDni() %>" name="DNIin" type="submit" class="btn btn-light border border-dark"  style="width:100px; height:35px">Borrar</button></td>
						</form>
						</tr>
								<% } %>
						</tbody>
