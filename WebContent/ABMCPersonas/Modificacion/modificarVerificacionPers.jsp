...
<%@page import="conexionSQL.DataRol"%>
...
<%
    ...
    try{
        ...
        p.setRol(dr.getById(r));

    }catch(Exception e){
        request.getRequestDispatcher("/WEB-INF/errorModificarPersona.jsp")
...
<body>
<form action="/TpJava/servletModificarPersonas" method="post">
		<input type="hidden" name="DNIin" value="<%=p.getDni()%>" />
		<input type="hidden" name="nomIn" value="<%=p.getNombre()%>" />
		<input type="hidden" name="apeIn" value="<%=p.getApellido()%>" />
		<input type="hidden" name="telIn" value="<%=p.getTelefono()%>" />
		<input type="hidden" name="IdRolIn" value="<%=request.getParameter("IdRolIn")%>" />
		<input type="hidden" name="valIn" value="<%=p.getValuacionPromedio()%>" />
		...
		<div class="container" align="left">
			<div class="row align-items-start">
				...
				<table class="table table-hover table-striped table-sm border-secondary border border-5 rounded-3 bg-light table-bordered">
					...
					<tr><td align="center"><font face="arial" size="5">Rol</font></td>					<td align="center"><font face="arial" size="5"><%=request.getParameter("IdRolIn") %></font> </td></tr>
			...
			<div class="row">
					<div class="col">
						<button value="<%=request.getParameter("DNIin") %>" name="DNIin" type="submit" class="btn btn-light border border-dark" style="width:170px; height:35px">Aceptar</button>
					</div>
			</form>
					<div class="col">	
